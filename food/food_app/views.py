...
from food_app.models import UserLogin,UserReg,AddTable,AddItem,AddCompany,ItemOrder,Orders,Pracel
...
def add_item(request):
    if request.method == "POST":
        ...
        # Generate a numeric unique identifier for the barcode
        unique_id = str(uuid.uuid4().int)[:12]  # Get the first 12 digits of the UUID integer

        # Create the item entry in the database with the barcode number
        ...
        ean = EAN(unique_id, writer=ImageWriter())  # EAN-13 requires 12 digits + 1 checksum digit

        # Define the path to save the barcode image
        barcode_dir = os.path.join(settings.BASE_DIR, 'barcode')
        os.makedirs(barcode_dir, exist_ok=True)
        barcode_path = os.path.join(barcode_dir, f'barcode_{unique_id}.png')

        try:
            # Save the barcode image
            barcode_full_path = barcode_path
            print(f"Attempting to save barcode to: {barcode_full_path}")
            ean.save(barcode_full_path)
            print(f"Barcode saved to {barcode_full_path}")

            # Update the item with the barcode path
            item.Barcode_image = os.path.join('barcode', f'barcode_{unique_id}.png')
            item.save()
        except Exception as e:
            print(f"Error saving barcode: {e}")
            return render(request, 'add_item.html', {'msg': f'Error saving barcode: {e}'})

        return render(request, 'add_item.html', {'msg': 'Added', 'barcode_path': item.Barcode_image})
...
def save_order(request):
    ...
    for item in bill_items:
        ...
        # Check if item with item_code already exists for the table_name
        existing_item_order = ItemOrder.objects.filter(table_name=table_name, item_code=item_code, bill_no=bill_no).first()

        if existing_item_order:
            # Update existing ItemOrder
            existing_item_order.item_cat = item_cat
            existing_item_order.item_name = item_name
            existing_item_order.price = price
            existing_item_order.gst = gst
            existing_item_order.qty = qty
            existing_item_order.tax_amt = tax_amt
            existing_item_order.total_gst = total_gst
            existing_item_order.total = total
        ...
        # Update or create corresponding record in Orders table
        existing_order = Orders.objects.filter(table_name=table_name, bill_no=bill_no, item_code=item_code).first()

        if existing_order:
            # Update existing Orders record
            existing_order.item_cat = item_cat
            existing_order.item_name = item_name
            existing_order.price = price
            existing_order.gst = gst
            existing_order.qty = qty
            existing_order.tax_amt = tax_amt
            existing_order.total_gst = total_gst
...
def sales_report(request):
    ...
    if request.method == 'POST' and selected_month:
        ...
        salesreport = Orders.objects.filter(
            bill_date__year=year, bill_date__month=month
        )

        # Calculate grand total and prepare sales data
        for sales in salesreport:
            ...
            grand_total += sales.total  # Add the total to the grand total

    return render(request, 'sales_report.html',
                  {'sales_data': sales_data, 'selected_month': selected_month, 'grand_total': grand_total})
...
def daily_report(request):
    ...
    formatted_current_date = current_date.strftime('%b %d, %Y')  # Nov 13, 2024 format
    ...
    if request.method == 'POST':
        ...
        if selected_date:
            ...
            formatted_selected_date = selected_date.strftime('%b %d, %Y')  # Nov 13, 2024 format
        else:
            orders = Orders.objects.none()  # No data if no date is selected
            formatted_selected_date = None

        # Calculate the grand total for the selected date's orders
